...
{
    public class RetroAchievementsJobDispatcherService(AppDbContext context) : IRetroAchievementsJobDispatcherService
    {
        ...
        public async Task QueueUnscheduledJobs()
        {
            ...
            var requestsToProcess = await context.RetroAchievementsApiData.Where(x => x.ProcessingStatus == ProcessingStatus.NotScheduled).CountAsync();
            ...
            var scheduledRequests = await context.RetroAchievementsApiData.Where(x => x.ProcessingStatus == ProcessingStatus.Scheduled || x.ProcessingStatus == ProcessingStatus.BeingProcessed).CountAsync();
            ...
            var unscheduledRequests = await context.RetroAchievementsApiData
